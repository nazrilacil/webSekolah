...
<div class="content">
    <div class="container mx-auto p-4 w-full h-full">
        <div class="bg-white shadow-md rounded-lg p-6">
            <h2 class="text-2xl font-semibold mb-4">Tambah Informasi</h2>

            <form action="" method="POST" enctype="multipart/form-data">
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700">Judul</label>
                    <input type="text" name="judul" placeholder="Judul" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-500" required>
                </div>

                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700">Keterangan</label>
                    <textarea name="keterangan" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-500" placeholder="Keterangan" id="keterangan" required></textarea>
                </div>
            ...
            if (isset($_POST['submit'])) {
                ...
                if (!in_array($formatfile, $allowedtype)) {
                    ...
                    if ($simpan) {
                        echo '<div class="mt-4 text-green-500">Simpan Berhasil</div>';
                    } else {
                        echo '<div class="mt-4 text-red-500">Gagal simpan ' . mysqli_error($conn) . '</div>';
                    }
                }
            }
            ?>
